...
3. **周辺の飲食店を表示** を押すと、半径 700m の飲食店（OpenStreetMap 登録分）が灰色の点で出ます。
   点をタップすると「採点する」「Google マップ」「経路」が選べます。
4. 登録済みの店は点数つきの青いピン、行きたいリストの店は赤枠つきで表示されます。
   ピンをタップすると「店ページ」と「経路」（Google マップの徒歩ルート）が開きます。

地図データは OpenStreetMap（© OpenStreetMap contributors）です。周辺店の取得は
Overpass API の公開サーバーを使うため、混雑時に数秒かかることがあります。
